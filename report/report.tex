...
\section{Context}
The idea we had when starting this available distributed fault tolerant software system was to create a database system that nobody has ever seen before. Since that is an immensely difficult task. On a single computer it is perfectly possible to read and write effectively. However when we distribute the services of a storage system things get more complicated. What we basically want is to have a storage system from which we can read and write data in a scalable, consistent and fault tolerant way (that is we do not want to notice faults.
...
An important aspect of our application is that parity storage is used on the application level instead of hardware level as with Redundant Array of Independent Disks (RAID).% TODO: add RAID reference http://nl.wikipedia.org/wiki/Redundant_Array_of_Independent_Disks
...
Message fields :) % @Karsten: I more or less already explained this in the storage section
